...
router.post('/nebulablock/generate-voice', async (request, response) => {
  try {
    ...
    if (!result.ok) {
      const text = await result.text();
      console.warn('MegaNova AI TTS request failed', result.statusText, text);
      return response.status(500).send(text);
    }
    ...
    console.error('MegaNova AI TTS generation failed', error);
    response.status(500).send('Internal server error');
  }
});
...
router.post('/nebulablock/models', async (request, response) => {
  try {
    ...
    if (!result.ok) {
      const text = await result.text();
      console.warn('MegaNova AI models request failed', result.statusText, text);
      return response.status(500).send(text);
    }
    ...
    console.error('MegaNova AI models fetch failed', error);
    response.status(500).send('Internal server error');
  }
});
