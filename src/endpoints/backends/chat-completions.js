...
/**
 * Sends a request to Nebula Block.
 * @param {express.Request} request Express request
 * @param {express.Response} response Express response
 */
async function sendNebulablockRequest(request, response) {
    ...
    if (!apiKey) {
        console.warn('Nebula Block key is missing.');
        return response.status(400).send({ error: true });
    }
    ...
    try {
        ...
        console.debug('Nebula Block request:', requestBody);
        ...
        if (request.body.stream) {
            ...
            if (!generateResponse.ok) {
                const errorText = await generateResponse.text();
                console.warn('Nebula Block returned error: ', errorText);
                const errorJson = tryParse(errorText) ?? { error: true };
                return response.status(500).send(errorJson);
            }
            const generateResponseJson = await generateResponse.json();
            console.debug('Nebula Block response:', generateResponseJson);
            return response.send(generateResponseJson);
        }
    }
    catch (error) {
        console.error('Error communicating with Nebula Block: ', error);
        if (!response.headersSent) {
            response.send({ error: true });
        } else {
            response.end();
...
router.post('/status', async function (request, statusResponse) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.NEBULABLOCK) {
...
router.post('/generate', function (request, response) {
    ...
    switch (request.body.chat_completion_source) {
        ...
        case CHAT_COMPLETION_SOURCES.NEBULABLOCK: return sendNebulablockRequest(request, response);
